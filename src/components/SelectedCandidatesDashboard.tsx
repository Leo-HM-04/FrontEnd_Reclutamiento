...
/**
 * ============================================================
 * SELECTED CANDIDATES DASHBOARD
 * ============================================================
 * Dashboard para gestión de candidatos seleccionados con oferta
 * - Solo candidatos con estado "offered" o "accepted"
 * - Vista detallada completa de cada candidato
 * - Upload de documentos (contratos, onboarding, etc.)
 * - Información de oferta y fechas
...
export default function SelectedCandidatesDashboard() {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'offered' | 'accepted'>('all');
  ...
  useEffect(() => {
    if (selectedProfile) {
      loadSelectedCandidates(selectedProfile.id);
    }
  }, [selectedProfile]);
  ...
  const loadProfiles = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Filtrar perfiles que tengan al menos un candidato con oferta
        const profilesWithSelected = await filterProfilesWithSelectedCandidates(allProfiles, token);
        
        console.log('✅ Perfiles con candidatos seleccionados:', profilesWithSelected.length);
        setProfiles(profilesWithSelected);
        
        if (profilesWithSelected.length > 0 && !selectedProfile) {
          setSelectedProfile(profilesWithSelected[0]);
        }
      } else {
  ...
  const filterProfilesWithSelectedCandidates = async (profiles: Profile[], token: string | null) => {
    ...
    for (const profile of profiles) {
      try {
        ...
        if (response.ok) {
          ...
          // Filtrar por offered o accepted
          const selectedApps = apps.filter((app: CandidateApplication) => 
  ...
  const loadSelectedCandidates = async (profileId: number) => {
    setLoadingCandidates(true);
    setCandidatesData([]);
    console.log('🟢 Cargando candidatos seleccionados para perfil:', profileId);
    ...
    try {
      ...
      if (appsResponse.ok) {
        ...
        const allApplications = appsData.results || appsData;
        
        // Filtrar solo offered y accepted
        const applications = allApplications.filter((app: CandidateApplication) => 
          app.status === 'offered' || app.status === 'accepted'
        );
        
        console.log('📋 Candidatos con oferta:', applications.length);
        ...
        console.log('✅ Candidatos cargados:', candidatesWithData.length);
        setCandidatesData(candidatesWithData.filter(c => c.candidate !== null));
      }
    } catch (error) {
      console.error('💥 Error loading selected candidates:', error);
      showNotification('Error al cargar candidatos seleccionados', 'error');
    } finally {
      setLoadingCandidates(false);
    }
  };
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      if (allSuccess) {
        ...
        if (selectedProfile) {
          await loadSelectedCandidates(selectedProfile.id);
        }
  ...
  const getFilteredCandidates = () => {
    return candidatesData.filter(c => {
      ...
      if (candidateSearchTerm) {
        ...
        const matchesSearch = c.candidate.full_name.toLowerCase().includes(searchLower) ||
      ...
      // Filtro por estado
      if (statusFilter !== 'all') {
        if (c.application.status !== statusFilter) return false;
      }
  ...
  const getStatusBadgeColor = (status: string) => {
    if (status === 'offered') return 'bg-green-100 text-green-800';
    ...
    <div className="h-screen flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Candidatos Seleccionados</h1>
            <p className="text-gray-600 mt-1">Gestiona candidatos con oferta extendida o aceptada</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-green-50 p-3 rounded-lg">
            <div className="text-green-600 text-sm font-medium">Perfiles Activos</div>
            <div className="text-xl font-bold text-gray-900">{profiles.length}</div>
          </div>
          <div className="bg-blue-50 p-3 rounded-lg">
            <div className="text-blue-600 text-sm font-medium">Total Seleccionados</div>
            <div className="text-xl font-bold text-gray-900">{candidatesData.length}</div>
          </div>
          <div className="bg-yellow-50 p-3 rounded-lg">
            <div className="text-yellow-600 text-sm font-medium">Oferta Extendida</div>
            <div className="text-xl font-bold text-gray-900">{candidatesData.filter(c => c.application.status === 'offered').length}</div>
          </div>
          <div className="bg-emerald-50 p-3 rounded-lg">
            <div className="text-emerald-600 text-sm font-medium">Oferta Aceptada</div>
            <div className="text-xl font-bold text-gray-900">{candidatesData.filter(c => c.application.status === 'accepted').length}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 bg-white border-r border-gray-200 flex flex-col">
          ...
          <div className="p-4 border-b border-gray-200">
            <h2 className="text-sm font-semibold text-gray-900 mb-3">PERFILES CON CANDIDATOS SELECCIONADOS</h2>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="text-center py-12 px-4">
                ...
                <p className="text-gray-600 text-sm">No hay perfiles con candidatos seleccionados</p>
              </div>
            ) : (
              <div className="p-2">
                {filteredProfiles
          ...
          {selectedProfile ? (
              ...
              <div className="flex-1 overflow-y-auto p-6">
                {loadingCandidates ? (
                  ...
                  <div className="text-center py-12">
                    <i className="fas fa-user-check text-gray-400 text-5xl mb-4" />
                    <p className="text-gray-600">No hay candidatos seleccionados para este perfil</p>
                  </div>
                ) : (
                  <>
                    {/* Select All */}
                              ...
                              <div className="flex-1 ml-4">
                                ...
                                <div className="flex items-center space-x-6 mt-4">
                                  <div className="flex items-center">
                                    <span className={`px-3 py-1 rounded-full text-sm font-semibold ${getStatusBadgeColor(candidateData.application.status)}`}>
                                      {candidateData.application.status === 'offered' ? 'Oferta Extendida' : 'Oferta Aceptada'}
                                    </span>
                                  </div>
                                  {candidateData.application.offer_date && (
                                    <div className="flex items-center">
                                      <i className="fas fa-calendar-check text-green-600 mr-2" />
                                      <span className="text-sm text-gray-600">
                                        Oferta: {new Date(candidateData.application.offer_date).toLocaleDateString()}
                                      </span>
                                    </div>
                                  )}
                                  {candidateData.application.match_percentage && (
                                    <div className="flex items-center">
                                      <i className="fas fa-chart-line text-green-600 mr-2" />
                          ...
                          {candidateData.expanded && (
                            <div className="p-6">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                ...
                                <div className="space-y-3">
                                  <h4 className="font-bold text-gray-900 flex items-center mb-4">
                                    <i className="fas fa-address-card text-green-600 mr-2" />
                                ...
                                <div className="space-y-3">
                                  <h4 className="font-bold text-gray-900 flex items-center mb-4">
                                    <i className="fas fa-graduation-cap text-green-600 mr-2" />
                                    Educación y Experiencia
                                  </h4>
                              ...
                              {candidateData.candidate.skills.length > 0 && (
                                <div className="mb-6">
                                  <h4 className="font-bold text-gray-900 flex items-center mb-3">
                                    <i className="fas fa-code text-green-600 mr-2" />
                                    Habilidades
                                  </h4>
                              ...
                              {candidateData.candidate.certifications.length > 0 && (
                                <div className="mb-6">
                                  <h4 className="font-bold text-gray-900 flex items-center mb-3">
                                    <i className="fas fa-certificate text-green-600 mr-2" />
                                    Certificaciones
                                  </h4>
                              ...
                              <div className="mb-6">
                                <div className="flex items-center justify-between mb-3">
                                  <h4 className="font-bold text-gray-900 flex items-center">
                                    <i className="fas fa-folder-open text-green-600 mr-2" />
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-gray-600">Selecciona un perfil para ver candidatos seleccionados</p>
              </div>
            </div>
          )}
        </div>
