    ...
    <div>
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-semibold text-gray-900">Estadísticas de Perfiles</h3>
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-2">{stats.total_profiles}</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-2">{stats.active_profiles}</p>
            </div>
            <div className="w-12 h-12 bg-white bg-opacity-20 rounded-full flex items-center justify-center">
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-2">{stats.completed_profiles}</p>
            </div>
            <div className="w-12 h-12 bg-white bg-opacity-20 rounded-full flex items-center justify-center">
        ...
        <div className="bg-red-50 border-l-4 border-red-500 p-4 mb-6">
          <div className="flex items-center">
            ...
            <p className="text-red-700">
              <span className="font-semibold">{stats.near_deadline}</span> perfiles próximos a vencer
            </p>
          </div>
        </div>
      )}
                ...
                <div className="flex items-center space-x-2">
                  <div className="w-24 bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${(count / stats.total_profiles) * 100}%`,
                      }}
                    ></div>
                  </div>
            ...
            {Object.entries(stats.by_service_type).map(([type, count]) => (
              <div key={type} className="flex items-center justify-between">
                <span className="text-gray-700 capitalize">
                  {type === "normal" ? "Servicio Normal" : "Servicio Especializado"}
                </span>
          ...
          <div className="space-y-3">
            {Object.entries(stats.by_modality).map(([modality, count]) => (
              <div key={modality} className="flex items-center justify-between">
                <span className="text-gray-700 capitalize">{modality}</span>
                <span className="font-semibold text-gray-900">{count}</span>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {stats.avg_positions_per_profile.toFixed(1)}
          </p>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <p className="text-xl font-bold text-gray-900">
            ${stats.avg_salary_range.min.toLocaleString()} - ${stats.avg_salary_range.max.toLocaleString()}
          </p>
          <p className="text-sm text-gray-600 mt-2">{stats.avg_salary_range.currency}</p>
        </div>
      </div>
            ...
            {stats.top_clients.map((client, index) => (
              ...
              >
                ...
                <span className="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-sm font-medium">
                  {client.profile_count} {client.profile_count === 1 ? "perfil" : "perfiles"}
                </span>
              </div>
            ))}
          </div>
